...
sequenceDiagram
    ...
    Scanner->>Scanner: _onScanSuccess(decodedText)<br/>
    Scanner->>App: Callback onQRDetected(decodedText)

    App->>App: handleQrCode(rawQrCode)<br/>
    App->>App: handleTourQrCode(rawQrCode)<br/>

    Note over App: Bước 1 — Tách mã QR
    App->>App: extractTourQrCode(rawQrCode)<br/><br/>Trích ?tour= hoặc ?qr= từ URL

    Note over App: Bước 2 — Gọi API lấy tour
    App->>API: resolveTourByQrCode(qrCode)<br/><br/>GET /api/tour/qr/{qrCode}
    API->>API: MapGet("/api/tour/qr/{qrCode}")<br/>
    API->>DB: Tìm tour → lấy POI theo thứ tự
    DB-->>API: { tour, pois[] }
    API-->>App: JSON { tour, pois[] }

    alt API lỗi hoặc offline
        App->>App: getFallbackTourByQrCode(qrCode)<br/><br/>Dùng POI demo trong bộ nhớ
    end

    Note over App: Bước 3 — Mở tour & hiện danh sách quán
    App->>App: openTourFromQr(payload, qrCode)<br/>
    App->>App: normalizeTourPayload(payload)<br/>
    App->>App: AppState.activeTour = {tour, pois, currentIndex: 0}
    App->>App: renderTourPoiList()<br/>
    App->>App: showPoiDetail(firstPoi)<br/>— Hiện quán đầu tiên
    App-->>User: Hiện danh sách quán + nút Trước/Tiếp

    Note over App: Bước 4 — Ghi thống kê
    App->>API: trackTourQrScan()<br/><br/>POST /api/analytics/event {eventType: "qr_scan"}

    Note over User,App: Chuyển tiếp giữa các quán
    User->>App: Bấm "Tiếp theo" hoặc "Trước"
    App->>App: goToTourStop(delta)<br/>
    App->>App: showPoiDetail(nextPoi)<br/>Hiện quán kế tiếp + phát audio
    App->>App: renderTourNavigation(poi)<br/><br/>Cập nhật "Điểm 2/5"
```
...
sequenceDiagram
    ...
    App->>App: changeLanguage(lang)<br/><br/>normalizeAppLanguage() 

    alt vi hoặc en
        App->>App: Dùng trực tiếp source text
    else Ngôn ngữ khác
        App->>GTranslate: translateWithCache(scope,id,text,src,target)<br/><br/>translateText() 
        GTranslate-->>App: Text đã dịch → RAM cache + localStorage debounce
    end

    App->>App: Render UI ngay bằng source/cache<br/>queuePoiTranslationWarmup() 
    User->>App: Bấm "Nghe thuyết minh"
    App->>App: getPoiScript(poi, lang)<br/>
    App->>TTS: playDirect(poiId, script, name)<br/>
    TTS->>TTS: _speak(text, lang)<br/>

    alt Có voice hệ thống
        TTS->>TTS: _speakWithWebSpeech()<br/>
        TTS-->>User: Phát audio
    else Không có voice
        TTS->>TTS: _speakWithGoogleTTS()<br/><br/>_ensureGoogleAudio() 
        TTS->>Proxy: GET /api/tts?lang=ko-KR&text=...
        Proxy-->>TTS: audio/mpeg + cache/range headers<br/>
        TTS-->>User: Phát audio fallback
    end

    TTS->>TTS: _trackListen(poiId)<br/>
```
...
sequenceDiagram
    ...
    Web->>API: loadDashboardData() <br/>GET /api/analytics/stats + top-pois + recent + poi/all
    API-->>Web: Dữ liệu thống kê
    Web->>Web: renderTopPoisChart() <br/>renderRecentEvents() 
    Web->>API: initHeatmap() <br/>GET /api/analytics/heatmap
    Web->>Web: startDashboardRefresh() <br/>poll lại mỗi 5 giây khi Dashboard active
...
sequenceDiagram
    ...
    User->>App: Mở web app
    App->>App: startPresenceHeartbeat()<br/>
    loop Mỗi 15 giây
        App->>API: sendPresenceHeartbeat()<br/><br/>POST /api/presence/heartbeat
        API->>Presence: Upsert(session)<br/>
        Presence-->>API: { sessionId, isKicked }
        API-->>App: { sessionId, kicked }
    end

    Admin->>CMS: Mở trang Online Users
    CMS->>API: loadOnlineUsers()<br/><br/>GET /api/admin/online-users
    API->>Presence: GetOnlineSessions()<br/>online window 45s
    API-->>CMS: { count, users[] }

    Admin->>CMS: Bấm Kick
    CMS->>API: kickOnlineUser(sessionId)<br/><br/>POST /api/admin/online-users/{sessionId}/kick
    API->>Presence: Kick(sessionId)

    App->>API: Heartbeat kế tiếp
    API-->>App: { kicked: true }
    App->>App: handleSessionKicked()<br/><br/>Dừng audio/GPS + khóa màn hình
```
