...
| BR-08 | Nếu MongoDB/API chưa sẵn sàng khi demo, frontend dùng dữ liệu POI mẫu để vẫn trình bày được bản đồ, đổi ngôn ngữ, QR và TTS. |
| BR-09 | Nếu Google Translate/TTS không khả dụng, app fallback về source `vi/en` và thông báo trạng thái thay vì để giao diện rỗng. |
| BR-10 | QR in tại quán encode URL đầy đủ `/index.html?qr=<POI_CODE>`; app cũng hỗ trợ QR chỉ chứa POI code để dễ test. |
...
sequenceDiagram
    ...
    Web->>QRGen: Tạo QR local từ URL Web App có ?qr=<POI_CODE>
    QRGen-->>Web: Trả về data URL hình QR; lỗi mới dùng QRServer fallback
